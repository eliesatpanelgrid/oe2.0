...
#########################################
imgnm=$(curl -s "https://v72.hdfreaks.cc/$device"/ | grep $device | grep -o 'href="[^"]*\.zip"' | sed 's/\.zip.*/.zip/' | sed 's/^.*openhdf/openhdf/' | head -n 1)
...
if wget -q --method=HEAD https://v72.hdfreaks.cc/current.php?open=$device;
 then
wget --show-progress -qO $ms/images/$imgnm https://v72.hdfreaks.cc/current.php?open=$device
else
