...
imgnm=teamblue-7.5-"$device"-"$date"_mmc.zip
echo "> $imgnm "
sleep 5
...
if wget -q --method=HEAD https://images.teamblue.tech/7.5-release/current.php?open=$device;
 then
wget --show-progress -qO $ms/images/$imgnm https://images.teamblue.tech/7.5-release/current.php?open=$device
else
