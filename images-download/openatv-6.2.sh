...
#########################################
imgnm=$(curl -s "https://images.mynonpublic.com/openatv/index.php?v=$version&open=$device" | grep -o "$device/.*\.zip" | tail -n 1 | sed "s/$device\///" | cut -d '>' -f1 | tr -d "'" | sed 's/\.zip.*/.zip/')
...
sleep 3
url="https://images.mynonpublic.com/openatv/$version/$device/$imgnm"

echo $url
echo "$url" > /tmp/url.txt

if wget -q --method=HEAD https://images.mynonpublic.com/openatv/$version/$device/$imgnm ;
 then
wget --show-progress -qO $ms/images/$imgnm https://images.mynonpublic.com/openatv/$version/$device/$imgnm
else
