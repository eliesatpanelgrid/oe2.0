...
PLUGIN_DIR="/usr/lib/enigma2/python/Plugins/Extensions/MyTranslator"

case "$ARCH" in
    *aarch64*|*arm64*)
        if [ "$PYTHON_VERSION" = "3.12" ]; then
            FILE_TO_EXTRACT="aarch64_py3.12.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.13" ]; then
            FILE_TO_EXTRACT="aarch64_py3.13.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.14" ]; then
            FILE_TO_EXTRACT="aarch64_py3.14.tar.gz"
        fi
        ;;
    *arm*)
        if [ "$PYTHON_VERSION" = "3.12" ]; then
            FILE_TO_EXTRACT="arm_py3.12.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.13" ]; then
            FILE_TO_EXTRACT="arm_py3.13.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.14" ]; then
            FILE_TO_EXTRACT="arm_py3.14.tar.gz"
        fi
        ;;
    *mips*)
        if [ "$PYTHON_VERSION" = "3.12" ]; then
            FILE_TO_EXTRACT="mipsel_py3.12.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.13" ]; then
            FILE_TO_EXTRACT="mipsel_py3.13.tar.gz"
        elif [ "$PYTHON_VERSION" = "3.14" ]; then
            FILE_TO_EXTRACT="mipsel_py3.14.tar.gz"
        fi
        ;;
    *)
        exit 1
...
if [ -f "$PLUGIN_DIR/$FILE_TO_EXTRACT" ]; then
    tar -xzf "$PLUGIN_DIR/$FILE_TO_EXTRACT" -C /
    rm -f $PLUGIN_DIR/*.tar.gz
else
    exit 1
fi
