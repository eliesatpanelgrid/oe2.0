#!/bin/sh
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/customchannelselection/customchannelselection.sh

# Configuration
#########################################
plugin="customchannelselection"
rm="CustomChannelSelection"
section="addons"
...
# 1. Try Enigma info file (Most accurate for Enigma2 images)
if [ -f /usr/lib/enigma.info ]; then
    ARCH_IPK=$(grep "^architecture=" /usr/lib/enigma.info | cut -d"=" -f2 | tr -d "'\"")
fi

# 2. Try OPKG arch configuration if step 1 failed
if [ -z "$ARCH_IPK" ] && [ -f /etc/opkg/arch.conf ]; then
...
# 3. Kernel fallback if both image files are missing
if [ -z "$ARCH_IPK" ]; then
    UNAME_M=$(uname -m)
    case "$UNAME_M" in
        aarch64|arm64)
...
# Define IPK filename and download URL matching the provided links
ipk_file="${package}_${ARCH_IPK}_py${PY}.ipk"
...
download_and_install_package() {
    print_message "> Downloading $plugin package please wait ..."
    sleep 3
    wget --no-check-certificate --show-progress "$url" -qO "$temp_dir/$ipk_file"
    ...
    if [ -f "$temp_dir/$ipk_file" ] && [ -s "$temp_dir/$ipk_file" ]; then
        ...
        if [ $install_status -eq 0 ]; then
            ...
            print_message "> $plugin package installation failed"
