...
# Detect Python version string (e.g., 3.14 or 3.13)
...
ipk_file="${package}_${version}_${ARCH}_py${PY_VER}.ipk"
...
download_and_install_package() {
  ...
  sleep 3
  wget --show-progress -qO $temp_dir/$ipk_file --no-check-certificate $url
  
  if [ -f $temp_dir/$ipk_file ]; then
    if [ "$OS" = "DreamOS" ]; then
      dpkg -i $temp_dir/$ipk_file > /dev/null 2>&1
    else
      opkg install --force-overwrite $temp_dir/$ipk_file > /dev/null 2>&1
    fi
    extract=$?
  else
    extract=1
