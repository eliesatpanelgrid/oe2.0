...
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/ipaudiopro/1.7/ipaudiopro.sh
#configuration
###########################################
...
detect_ffmpeg() {
    ...
    if [ -n "$RAW_VERSION" ]; then
        ...
        echo "Could not detect FFmpeg version"
        exit 1
    fi
}
...
END
    ...
    if [ "x$ARCH" = "xunknown" ]; then
        ...
        if echo $(uname -m) | grep -q "armv7l"; then
            CPU_PART=$(grep "CPU part" /proc/cpuinfo | head -n1 | awk '{print $NF}')
            case "$CPU_PART" in
                "0xc09")
...
site="https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/ipaudiopro/1.7"
temp_dir="/tmp"
...
echo "*******************************************"
    ...
    # Executed directly without silence redirect so all output prints to terminal
