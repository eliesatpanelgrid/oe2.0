#!/bin/sh
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/tivimatee2/tivimatee2.sh
...
PLUGIN=/usr/lib/enigma2/python/Plugins/Extensions/TiviMateE2
if [ -d "$PLUGIN" ]; then
    rm -rf "$PLUGIN/__pycache__" 2>/dev/null || true
    find "$PLUGIN" -type f \( -name '*.pyc' -o -name '*.pyo' \) -exec rm -f {} \; 2>/dev/null || true
fi
