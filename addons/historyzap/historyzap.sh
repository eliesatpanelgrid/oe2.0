#!/bin/sh
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/historyzap/historyzap.sh

# Configuration
#########################################
plugin="historyzap"
rm="HistoryZapSelector"
section="addons"
...
check_ver() {
    pyver=$($1 -V 2>&1 | cut -d\  -f2 | awk -F "." '{print $1$2}')
}
...
if [ $? -eq 0 ];then
    if [ -d "/etc/dpkg" ]; then
        linkR="$path/Components/Renderer/EventListDisplay.py"
        linkC="$path/Components/Converter/EventList.py"
        if [ ! -L $linkR ]; then
            ln -s $destination/modules/EventListDisplay.py $linkR > /dev/null 2>&1
        fi
        if [ ! -L $linkC ]; then
            ln -s $destination/modules/EventList.py $linkC > /dev/null 2>&1
        fi
    fi
    find /tmp/ -type d -name '[0-9]*' -exec rm -r {} + -depth > /dev/null 2>&1
    find $(pwd)/ /tmp/ -name "enigma2-plugin-extensions-historyzap_*.*" -type f -exec rm {} + > /dev/null 2>&1
    type systemctl > /dev/null 2>&1 && path="dpkg" || path="opkg"
