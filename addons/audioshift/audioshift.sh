#!/bin/bash
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/addons/audioshift/audioshift.sh

# Configuration
###########################################
package="enigma2-plugin-extensions-audioshift"
version="1.2.4-r4"
site="https://github.com/eliesatpanelgrid/oe2.0/releases/download/1.2.4"
temp_dir="/tmp"
...
# Construct exact target file name based on releases available:
# 1) enigma2-plugin-extensions-audioshift-1.2.4-r4-aarch64-py3.14.ipk
# 2) enigma2-plugin-extensions-audioshift-1.2.4-r4-cortexa15hf-neon-vfpv4-py3.13.ipk
# 3) enigma2-plugin-extensions-audioshift-1.2.4-r4-cortexa15hf-neon-vfpv4-py3.14.ipk
plugin="${package}-${version}-${ARCH}-${pp}"
...
# Determine and check URL (supports redirected release links via wget/curl HEAD)
###########################################
url="$site/$plugin.$plugin_extension"

if wget --spider -q "$url" 2>/dev/null || curl -sI "$url" | grep -q "302 Found\|200 OK"; then
...
# Check and install or remove package
###########################################
check_and_install_package() {

plugin_path="/usr/lib/enigma2/python/Plugins/Extensions/AudioShift"
...
printf "Dependency check and installation completed.\n"
    ...
    wget -q --show-progress -L "$url" -P "$temp_dir"
    if [ $? -ne 0 ]; then
