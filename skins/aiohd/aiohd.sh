#!/bin/sh
#https://raw.githubusercontent.com/eliesatpanelgrid/oe2.0/main/skins/aiohd/aiohd.sh
...
# Skip if running during offline image creation
if [ -n "${D:-}" ] || [ -n "${IPKG_INSTROOT:-}" ]; then exit 0; fi

# Update active bootlogo if AIOHD is set as primary skin
python3 - <<'AIO_BOOTLOGO_PY' || true
import shutil
from pathlib import Path

settings = Path('/etc/enigma2/settings')
source = Path('/usr/share/enigma2/AIOHD/bootlogo.mvi')
target = Path('/usr/share/bootlogo.mvi')

if settings.exists() and source.is_file():
    if any(line.startswith('config.skin.primary_skin=AIOHD/') for line in settings.read_text().splitlines()):
        backup = Path('/usr/share/bootlogo.mvi.before-aiohd-next')
